...
const AddAchievementModal = ({ isOpen, onClose, onAchievementCreated }: AddAchievementModalProps) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.success) { onAchievementCreated(); handleClose() }
      else setError(result.message || 'Failed to save achievement')
    } catch (err) {
  ...
  const handleClose = () => {
    setFormData({ title: '', description: '', achievementType: '', startDate: '', endDate: '', isPresent: false, proofImage: null })
    setImagePreview(null); setError(null); onClose()
  }
    ...
    >
      ...
      >
        ...
        <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {/* Review-workflow note — owner-facing reminder so submissions
              don't feel like silent voids while waiting for a director. */}
          <div style={{ background: 'rgba(255,199,0,0.12)', border: '1px solid rgba(255,199,0,0.35)', borderRadius: 'var(--r)', padding: '10px 14px', color: '#7A5A00', fontFamily: 'var(--eina)', fontSize: 12.5, lineHeight: 1.55, display: 'flex', gap: 8 }}>
            <span aria-hidden style={{ fontSize: 14, lineHeight: 1 }}>⏳</span>
            <span>
              <strong>A director will review this before it goes public.</strong>{' '}
              You'll see it on your own profile as <em>"pending review"</em> until then.
              Add a proof image — it makes approval much faster.
            </span>
          </div>
