...
import { generateBlogGraphic, downloadBlogGraphic, type BlogData, type BlogFormat, type BlogResult } from './blogGenerator'
...
// generate/regenerate so every export is fresh. Always ends on the
// "READ NOW · ngoaquaterra.com/blogs" CTA.
export default function BlogStudioModal({ data, onClose }: BlogStudioModalProps) {
  ...
  const run = useCallback(async (fmt: BlogFormat) => {
    const id = ++reqId.current
    setBusy(true); setError(null)
    try {
      const res = await generateBlogGraphic(data, fmt)
  ...
  return (
    ...
    >
      <div
        ...
        style={{ width: '100%', maxWidth: 460, background: 'var(--card)', borderRadius: 22, border: '2px solid var(--ink)', overflow: 'hidden', maxHeight: '92dvh', display: 'flex', flexDirection: 'column', animation: 'sheetUp 0.22s cubic-bezier(0.2,0,0,1)' }}
      >
        {/* Header */}
            ...
            {FORMATS.map(f => {
              ...
              return (
                <button
                  ...
                  aria-pressed={active}
                  style={{ flex: 1, padding: '9px 4px', borderRadius: 10, cursor: 'pointer', border: 'none', background: active ? 'var(--ink)' : 'transparent', color: active ? 'var(--card)' : 'var(--ink-2)', transition: 'background 0.16s, color 0.16s', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}
                >
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '14px 18px 4px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
          <div style={{ position: 'relative', aspectRatio: FORMATS.find(f => f.value === format)!.ratio, height: 'min(46dvh, 420px)', borderRadius: 14, overflow: 'hidden', border: '2px solid var(--line)', background: 'var(--bg-2)', display: 'grid', placeItems: 'center', flexShrink: 0, transition: 'aspect-ratio 0.2s' }}>
            {result && (
