...
// @ts-expect-error - poly-decomp has no type declarations
...
const Gravity = forwardRef<GravityRef, GravityProps>(
  ({ children, debug = false, gravity = { x: 0, y: 1 }, grabCursor = true,
    resetOnResize = true, addTopWall = true, autoStart = true, className, ...props }, ref) => {
    ...
    const registerElement = useCallback((id: string, element: HTMLElement, props: MatterBodyProps) => {
      ...
      if (props.bodyType === 'circle') {
        ...
        body = Bodies.circle(x, y, radius, { ...props.matterBodyOptions, angle, render: renderOpts })
        ...
        body = Bodies.fromVertices(x, y, vertexSets, { ...props.matterBodyOptions, angle, render: renderOpts })
      } else {
        body = Bodies.rectangle(x, y, width, height, { ...props.matterBodyOptions, angle, render: renderOpts })
      }
